...
class IntcodeComputer
{
        ...
        for ($cycle = 0; $cycle < 1000000; $cycle++) {
            ...
            switch ($opcode) {
                ...
                case self::OPCODE_GET:
                    ...
                    $this->pointer += 2;
                    $response = $this->addToOutputBuffer($output);
                    if ($response) {
                        return $response;
                    }
                    break;
                ...
                case self::OPCODE_EXIT:
                    ...
                    return new Response(Response::STATUS_CODE_HALT);
                    break;
                default:
    ...
    private function addToOutputBuffer(int $output): ?Response
    {
        $this->outputBuffer[] = $output;
        if (\count($this->outputBuffer) % 3 === 0) {
            $tile = new Tile($this->outputBuffer[0], $this->outputBuffer[1], $this->outputBuffer[2]);
            $this->output = array_merge($this->output, $this->outputBuffer);
            $this->outputBuffer = [];
            return new Response(Response::STATUS_CODE_OUTPUT, $tile);
        }
